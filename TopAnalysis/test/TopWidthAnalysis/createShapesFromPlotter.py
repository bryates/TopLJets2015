...
def main():
    ...
    parser.add_option('-w', '--wids',      dest='widList',   help='signal widths',                                    default='1.0,2.0,3.0,4.0',type='string')
    ...
    parser.add_option(      '--lfs',       dest='lfsInput',  help='lepton final states to consider',    default='highptEE2b,highptEM2b,highptMM2b',  type='string')
    ...
    for lfs in lfsList :
        ...
        for cat,wid in [(cat,wid) for cat in catList for wid in modWidList]:
            ...
            _,genVarShapes = getMergedDists(fIn,('%s_%s_'%(lfs,opt.dist)),(rawSignalList if opt.addSigs else None),('top' if opt.addSigs else ''),widList,nomWid,systSignalList)
