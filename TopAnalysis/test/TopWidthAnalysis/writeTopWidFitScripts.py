...
def prepareFitScript(datacard,POIs,unblind=False):
    ...
    fitScriptUrl='%s/runCombine_%s_%s_%s.sh'%(baseDir,cat,lfs,wid)
    ...
    fitScript.write('text2workspace.py %s -m 172.5')                    % os.path.basename(datacard))
    fitScript.write('--PO verbose --PO altSignal=%s --PO muFloating ')  % wid
    fitScript.write('-o %s_%s.root\n')                                  % (lfs,wid)
    ...
    for parameter in POIs:
        ...
        if parameter=='r':
            fitScript.write('\n## max likelihood fit\n')
            fitScript.write('echo \"Running MultiDimFit for r\"\n')
            fitScript.write('combine %s_%s.root -M MultiDimFit -m 172.5 -t -1 --expectSignal=1 -m 0\n'%(lfs,wid))
