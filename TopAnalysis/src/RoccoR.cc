...
CrystalBall::CrystalBall(double m_, double s_, double a_, double n_){
  init(m_, s_, a_, n_);
}

void CrystalBall::init(double m_, double s_, double a_, double n_){

  pi    = TMath::Pi();
  SPiO2 = sqrt(TMath::Pi()/2.0);
  S2    = sqrt(2.0);
...
double CrystalBall::pdf(double x){ 
  double d=(x-m)/s;
...
double CrystalBall::cdf(double x){
  double d = (x-m)/s;
...
double CrystalBall::invcdf(double u){
