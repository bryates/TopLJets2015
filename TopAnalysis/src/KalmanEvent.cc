  ...
  for(int ij=0; ij<ev_.nj; ij++) {
    ...
    for(int ipf = 0; ipf < ev_.nkpf; ipf++) {
      ...
      */
      ...
      /*
      if(ev_.k_l3d[ipf]/ev_.k_sigmal3d[ipf] > l3dsig_) {
        pftk.setEpoch("BCDEF", true);
        pftk.setEpoch("GH", true);
      }
      */
      tmpj.addTrack(pftk);
