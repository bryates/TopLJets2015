...
void getHist(TString fname, TH1F *&cent, int sample, bool doLerp=true) {
  ...
  RooPlot *ptfrac;
  if(fname.Contains("_xb") || fname.Contains("_inc")) {
...
if(sample==2) fname.ReplaceAll(".root",TString::Format("_inc.root"));
...
fname.ReplaceAll("172v5_fitdown","Data");
...
/*
fname.ReplaceAll("Data","172v5_lepcentral");
f = TFile::Open(fname);
w = (RooWorkspace*)f->Get("w");
frame = w->var("ptfrac")->frame();
sigData = (RooDataSet*)w->data("sigData");
