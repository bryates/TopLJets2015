...
#include "/afs/cern.ch/user/b/byates/TopAnalysis/LJets2015/2016/mtop/splot_jpsi.C"
...
TString report("");
TString json("\"sim\" :      [");
float chi2_jpsi_toy_test(TH1F *&data, TString sample="d0", TString tune="", TString name="", float num=0.855, int iteration=0);
//float chi2_jpsi_toy_test(TH1F *&data, TH1F *&mc, TString sample="d0", TString tune="", TString name="", float num=0.855);
void run_chi2_jpsi_toy(TString, int iteration);
//void run_chi2_jpsi_toy(std::vector<TH1F*>&, std::vector<TH1F*>&, TString);
RooRealVar ptfrac;
...
bin = {0-0.025, 0.2-0.025, 0.4-0.025, 0.55-0.025, 0.6-0.025, 0.65-0.025, 0.7-0.025, 0.75-0.025, 0.8125-0.025, 0.88-0.025, 0.9425-0.025, 1.0};
...
if(toyData) {
  splot_jpsi(pdata, TString::Format("toyData%d",iteration), false, "", epoch, false);
...
void chi2_jpsi_toy(TString set="", int queue=0) {
  int max(10);
  for(int i = 0; i < max; i++) {
    std::cout << std::endl << "iteration " << i+1 << "/" << max << std::endl << std::endl;
    run_chi2_jpsi_toy(set, i + max*queue);
  }
/*
  run_chi2_jpsi_toy("isr-down" ndata, nmc);
  run_chi2_jpsi_toy("isr-up" ndata, nmc);
  run_chi2_jpsi_toy("fsr-down" ndata, nmc);
  run_chi2_jpsi_toy("fsr-up" ndata, nmc);
  run_chi2_jpsi_toy("uedown" ndata, nmc);
  run_chi2_jpsi_toy("ueup" ndata, nmc);
  //run_chi2_jpsi_toy("erdON" ndata, nmc);
  run_chi2_jpsi_toy("GluonMove_erdON" ndata, nmc);
  //run_chi2_jpsi_toy("QCD_erdON" ndata, nmc);
  ...
  for(auto & it : syst) {
    run_chi2_jpsi_toy("down_"+it ndata, nmc);
    run_chi2_jpsi_toy("up_"+it ndata, nmc);
  }
  run_chi2_jpsi_toy("hdampdown" ndata, nmc);
  run_chi2_jpsi_toy("hdampup" ndata, nmc);
  run_chi2_jpsi_toy("tpt" ndata, nmc);
  run_chi2_jpsi_toy("bkg" ndata, nmc);
  run_chi2_jpsi_toy("as117" ndata, nmc);
  run_chi2_jpsi_toy("as119" ndata, nmc);
  run_chi2_jpsi_toy("m166v5" ndata, nmc);
  run_chi2_jpsi_toy("m169v5" ndata, nmc);
  run_chi2_jpsi_toy("m171v5" ndata, nmc);
  run_chi2_jpsi_toy("m173v5" ndata, nmc);
  run_chi2_jpsi_toy("m175v5" ndata, nmc);
  run_chi2_jpsi_toy("m178v5" ndata, nmc);
*/
...
void run_chi2_jpsi_toy(TString name="", int iteration=0) {
//void run_chi2_jpsi_toy(std::vector<int> &nentries, std::vector<int> &nmc, TString name="") {
gROOT->Reset();
low=999.;
high=0;
//name=lname;
...
for(auto & it : tune) {
  ...
  /*
  float chi = chi2_jpsi_toy_test(data[0], "jpsi", it, name, param[pos]);
  chi += chi2_jpsi_toy_test(data[1], "d0", it, name, param[pos]);
  chi += chi2_jpsi_toy_test(data[2], "jpsi", it, name, param[pos]);
  */
  //if(hists[pos] != nullptr) std::cout << hists[pos]->GetTitle() << std::endl;
  float chi = chi2_jpsi_toy_test(data[1], "jpsi", it, name, param[pos], iteration);
  //float chi = chi2_jpsi_toy_test(data[1], hists[pos], "d0", it, name, param[pos]);
...
chiTest->Fit("pol3","FSMEQRW","",0.6,1.15);//0.976);
...
*/
float min = chiTest->GetFunction("pol3")->GetMinimumX(0.6,1.25);//1.075);
...
float err = chiTest->GetFunction("pol3")->GetX(chimin+1,0.6,1.25);//1.075);
...
if(fullpt) name += "_jpT";
//c1->SaveAs(TString::Format("chi2_jpsi%s%d_toy.pdf",name.Data(),iteration));
//c1->SaveAs(TString::Format("chi2_jpsi%s%d_toy.png",name.Data(),iteration));
...
shiftData->GetXaxis()->SetTitle("J/#Psi #it{p}_{T} / #Sigma #it{p}_{T}^{ch}");
delete mc2;
mc->Scale(1./mc->Integral());
if(toy) shiftData->Scale(1./shiftData->Integral());
setupPad()->cd();
tdr(mc, epoch);
if(fullpt) mc->GetXaxis()->SetTitle("J/#Psi #it{p}_{T} / jet #it{p}_{T}");
mc->Draw();
tdr(mc, epoch);
if(epoch==0) {
gStyle->SetOptStat(0);
...
shiftData->Draw();
tdr(shiftData, epoch);
//c1->SaveAs(TString::Format("www/meson/morph/ptfrac/ptfrac_signal_toy_BCDEFGH_jpsi%d.pdf",iteration));
//c1->SaveAs(TString::Format("www/meson/morph/ptfrac/ptfrac_signal_toy_BCDEFGH_jpsi%d.png",iteration));
}
}
...
if(num==0) num=0.855;
if(name=="") name="172v5";
TString mcvname(TString::Format("mcVdata_%s_%d_%s_jpsi_%d",name.Data(),(int)(num*1000), epoch_name[epoch].Data(), iteration));
