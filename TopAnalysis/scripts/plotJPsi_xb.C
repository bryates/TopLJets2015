...
void plotJPsi_xb(TString sample="172v5", int epoch=0, bool fin=false, float best=0.) {
/*
std::vector<TString> tune = {"_700", "_ddown", "", "_ccentral", "_fit"};
std::vector<float> param = {0.700, 0.800, 0.855,0.900, 0.833};
...
name = TString::Format("LJets2015/2016/mtop/sPlot/sPlot/TopMass_%s%s_sPlot_jpsi.root",sample.Data(),tune[i].Data());
if(name.Contains("fit")) name.ReplaceAll(TString::Format("_%s%s_", sample.Data(), tune[i].Data()), "_172v5_");
name.ReplaceAll("jpsi.root","jpsi1_xb.root");
...
*/
std::cout << "Grabbing hist" << std::endl;
TH1F *h1 = (TH1F*)f->Get("ptfrac_signal_hist");
TH1F *b1 = (TH1F*)f->Get("ptfrac_bkg_hist");
std::cout << b1->Integral() << std::endl;
h1->SetDirectory(0);
b1->SetDirectory(0);
//h1->Add((TH1F*)b1->Clone());
std::cout << name << std::endl;
f->Close();
name.ReplaceAll("jpsi1.root","jpsi2.root");
f = TFile::Open(name);
/*
...
h2->SetDirectory(0);
std::cout << "Adding hist" << std::endl;
h1->Add((TH1F*)h2->Clone());
TH1F *b2 = (TH1F*)f->Get("ptfrac_bkg_hist");
b2->SetDirectory(0);
std::cout << b2->Integral() << std::endl;
//h1->Add((TH1F*)b2->Clone());
...
//ptfrac_hist->plotOn(ptfrac_mc);
  ...
  tmp.back()->SetMarkerColor(color[i]);
...
TString name(TString::Format("LJets2015/2016/mtop/sPlot/sPlot/TopMass_Data_sPlot_jpsi1_xb.root"));
f = TFile::Open(name);
std::cout << name << std::endl;
/*
...
d1->SetDirectory(0);
TH1F *db1 = (TH1F*)f->Get("ptfrac_bkg_hist");
db1->SetDirectory(0);
//d1->Add((TH1F*)db1->Clone());
f->Close();
name.ReplaceAll("jpsi1.root","jpsi2.root");
std::cout << name << std::endl;
f = TFile::Open(name);
...
d2->SetDirectory(0);
d1->Add((TH1F*)d2->Clone());
TH1F *db2 = (TH1F*)f->Get("ptfrac_bkg_hist");
db2->SetDirectory(0);
//d1->Add((TH1F*)db2->Clone());
...
if(fin) {
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi" + ext + b + "_jpsi_final.png");
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi" + ext + b + "_jpsi_final.pdf");
}
else {
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi_sig" + ext + b + "_jpsi.png");
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi_sig" + ext + b + "_jpsi.pdf");
}
