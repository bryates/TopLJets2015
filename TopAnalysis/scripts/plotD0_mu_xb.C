...
void plotD0_mu_xb(TString sample="172v5", int epoch=0, bool fin=false, float best=0.) {
/*
...
name.ReplaceAll("d0_mu_tag_mu.root","d0_mu_tag_mu1_inc.root");
...
std::cout << name << std::endl;
f->Close();
name.ReplaceAll("d0_mu_tag_mu1","d0_mu_tag_mu2");
f = TFile::Open(name);
if(f==nullptr) continue;
/*
...
//ptfrac_hist->plotOn(ptfrac_mc);
  ...
  if(i==0) {
    ...
    tmp.back()->GetYaxis()->SetRangeUser(0,0.42);
    /*
...
TString name(TString::Format("LJets2015/2016/mtop/sPlot/sPlot/TopMass_Data_sPlot_d0_mu_tag_mu1_inc.root"));
f = TFile::Open(name);
std::cout << name << std::endl;
/*
...
ratio->SetTitle("");
ratio->GetXaxis()->SetTitle("(D^{0} p_{T} + #mu p_{T}) / #Sigma p_{T}^{ch}");
...
if(fin) {
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_d0mu" + ext + b + "_d0mu_final.png");
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_d0mu" + ext + b + "_d0mu_final.pdf");
}
else {
