...
void plotJPsi(TString sample="172v5", int epoch=0, bool fin=false, float best=0.) {
/*
...
name = TString::Format("LJets2015/2016/mtop/sPlot/sPlot/TopMass_%s%s_sPlot_jpsi1_xb.root",sample.Data(),tune[i].Data());
...
//ptfrac_hist->plotOn(ptfrac_mc);
  ...
  if(i==0) {
    ...
    tmp.back()->GetYaxis()->SetRangeUser(0,0.2);
    /*
...
f->Close();
//ptfrac_data_hist->plotOn(ptfrac_data, RooFit::Binning(bins));
//tmp.push_back((TH1F*)convert(ptfrac_data,false,bin));
tmp.push_back(d1);
//tmp.push_back(convert((RooPlot*)f->Get("ptfrac_signal")));
tmp.back()->Scale(1./tmp.back()->Integral());
//tmp.back()->Rebin(2);
...
ratio->SetTitle("");
ratio->GetXaxis()->SetTitle("J/#Psi p_{T} / #Sigma p_{T}^{ch}");
...
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi" + ext + b + "_jpsi.png");
c1->SaveAs("LJets2015/2016/mtop/www/meson/morph/weights_jpsi" + ext + b + "_jpsi.pdf");
}
