...
TFile *f = new TFile(base + "data/era2016/pf_tracks_jpsi.root","RECREATE");
...
dataB->Draw("jpsi_mu1_pt:jpsi_mu1_eta>>hB","meson_id==443 && (jpsi_mass<2.987 || jpsi_mass>3.207)","colz");
TH2F *tmpB = (TH2F*)hB->Clone("tmpB");
dataB->Draw("jpsi_mu2_pt:jpsi_mu2_eta>>hB","meson_id==443 && (jpsi_mass<2.987 || jpsi_mass>3.207)","colz");
hB->Add(tmpB);
hB->SetName("jpsimu1B");
hB->SetTitle("J/#psi #mu");
hB->Draw("colz text e");
hB->Write();
delete tmpB;

dataG->Draw("jpsi_mu1_pt:jpsi_mu1_eta>>hG","meson_id==443 && (jpsi_mass<2.987 || jpsi_mass>3.207)","colz");
TH2F *tmpG = (TH2F*)hG->Clone("tmpG");
dataG->Draw("jpsi_mu2_pt:jpsi_mu2_eta>>hG","meson_id==443 && (jpsi_mass<2.987 || jpsi_mass>3.207)","colz");
hG->Add(tmpG);
hG->SetName("jpsimu1G");
hG->SetTitle("J/#Psi #mu");
hG->Draw("colz text e");
hG->Write();
delete tmpG;
ra = (TH2F*)hB->Clone();
ra->Divide(hB, hG, 1./19712.86, 1./16146.178);
ra->SetName("mu_eta_pt");
ra->SetTitle("");
ra->GetXaxis()->SetTitle("N_{B-F} / N_{GH} 1/(L_{B-F} /L_{GH}) J/#Psi #mu #eta");
ra->GetYaxis()->SetTitle("N_{B-F} / N_{GH} 1/(L_{B-F} /L_{GH}) J/#Psi #mu p_{T}");
//Testing errors, ROOT seems to be correct if Sumw2 is OFF
for(int i = ra->GetYaxis()->GetNbins(); i > 0; i--) {
  for(int j = 1; j < ra->GetXaxis()->GetNbins(); j++) {
    int bin = ra->GetBin(j,i);
