...
void splot_d0(TH1F *&ptfrac_signal, TString mass="172.5", bool isData=false, TString fragWeight="", int ep=0, bool jpT=false) {
...
*/
    ...
    for(int j=0; j<ev.nmeson; j++) {
      ...
      if(ev.nmeson>1) {
        ...
        if(ev.nmeson > j+2) //entries are 2 apart (j=0 epoch 1 j=1 epoch 2, j=2 epoch 1 j=3 epoch 2
          if(ev.d0_pi_pt[j] == ev.d0_k_pt[j+2] && ev.d0_k_pt[j] == ev.d0_pi_pt[j+2]) { scale *= 0.5; }
        pi = ev.d0_pi_pt[j];
        k = ev.d0_k_pt[j];
      }
      */
      ...
      if(mass.Contains("toyData")) {
        ...
        */
        TRandom3 *rand = new TRandom3(0);
        //if(epoch==1 && rand->Uniform(0, 1) > 0.25) continue;
        //if(epoch==2 && rand->Uniform(0, 1) > 0.17) continue;
  ...
  */
  ...
  // Construct Gaussian1 PDF for signal
  RooRealVar sigma("sigma","sigma", 0.018, 0.005, 0.5);
  RooRealVar ngsig("ngsig","ngsignal", 7000, 100, 10000000);
  RooGaussian gauss("gauss","gauss", d0_mass, mean, sigma);
...
void splot_d0(TString mass="172.5", bool isData=false, TString fragWeight="", int ep=0, bool jpT=false) {
  TH1F *pdata;
  splot_d0(pdata, mass, isData, fragWeight, ep, jpT);
  delete pdata;
}
