...
TString report("");
TString json("\"sim\" :      [");
float chi2_d0_mu_toy_test(TH1F *&data, TString sample="d0", TString tune="", TString name="", float num=0.855, int iteration=0);
//float chi2_d0_mu_toy_test(TH1F *&data, TH1F *&mc, TString sample="d0", TString tune="", TString name="", float num=0.855);
void run_chi2_d0_mu_toy(TString, int iteration);
//void run_chi2_d0_mu_toy(std::vector<TH1F*>&, std::vector<TH1F*>&, TString);
RooRealVar ptfrac;
...
if(toyData) {
  ...
  auto tmpf = new TFile(TString::Format("/eos/cms/store/group/phys_top/byates/sPlot/TopMass_172v5_sPlot_%s%d_inc.root",sample.Data(),epoch));
  auto tmph = (TH1F*)tmpf->Get("ptfrac_bkg_hist");
  auto tmps = (TH1F*)tmpf->Get("ptfrac_signal_hist");
  std::cout << "scaling to " << pdata->Integral() / tmps->Integral() << std::endl;
  pdata->Add((TH1F*)tmph->Clone(), -pdata->Integral() / tmps->Integral());
  delete tmph;
  delete tmps;
  tmpf->Close();
  delete tmpf;
...
if(epoch>0) fname.ReplaceAll(".root",TString::Format("_inc.root"));
...
void chi2_d0_mu_toy(TString set="", int queue=0) {
  int max(10);
  for(int i = 0; i < max; i++) {
    std::cout << std::endl << "iteration " << i+1 << "/" << max << std::endl << std::endl;
    run_chi2_d0_mu_toy(set, i + max*queue);
  }
/*
  run_chi2_d0_mu_toy("isr-down" ndata, nmc);
  run_chi2_d0_mu_toy("isr-up" ndata, nmc);
  run_chi2_d0_mu_toy("fsr-down" ndata, nmc);
  run_chi2_d0_mu_toy("fsr-up" ndata, nmc);
  run_chi2_d0_mu_toy("uedown" ndata, nmc);
  run_chi2_d0_mu_toy("ueup" ndata, nmc);
  //run_chi2_d0_mu_toy("erdON" ndata, nmc);
  run_chi2_d0_mu_toy("GluonMove_erdON" ndata, nmc);
  //run_chi2_d0_mu_toy("QCD_erdON" ndata, nmc);
  ...
  for(auto & it : syst) {
    run_chi2_d0_mu_toy("down_"+it ndata, nmc);
    run_chi2_d0_mu_toy("up_"+it ndata, nmc);
  }
  run_chi2_d0_mu_toy("hdampdown" ndata, nmc);
  run_chi2_d0_mu_toy("hdampup" ndata, nmc);
  run_chi2_d0_mu_toy("tpt" ndata, nmc);
  run_chi2_d0_mu_toy("bkg" ndata, nmc);
  run_chi2_d0_mu_toy("as117" ndata, nmc);
  run_chi2_d0_mu_toy("as119" ndata, nmc);
  run_chi2_d0_mu_toy("m166v5" ndata, nmc);
  run_chi2_d0_mu_toy("m169v5" ndata, nmc);
  run_chi2_d0_mu_toy("m171v5" ndata, nmc);
  run_chi2_d0_mu_toy("m173v5" ndata, nmc);
  run_chi2_d0_mu_toy("m175v5" ndata, nmc);
  run_chi2_d0_mu_toy("m178v5" ndata, nmc);
*/
...
void run_chi2_d0_mu_toy(TString name="", int iteration=0) {
//void run_chi2_d0_mu_toy(std::vector<int> &nentries, std::vector<int> &nmc, TString name="") {
gROOT->Reset();
low=999.;
high=0;
//name=lname;
...
std::vector<float> param = {0.655, 0.700, 0.725, 0.755, 0.775, 0.800, 0.825, 0.875, 0.900, 0.925, 0.955, 0.975, 1.055};
//std::vector<TString> tune = {"_sdown", "_700", "_725", "_down", "_dddown", "_ddown", "_scentral", "", "_cccentral", "_ccentral", "_925", "_central", "_uuup", "_up" };
...
for(auto & it : tune) {
  ...
  /*
  float chi = chi2_d0_mu_toy_test(data[0], "jpsi", it, name, param[pos]);
  chi += chi2_d0_mu_toy_test(data[1], "d0", it, name, param[pos]);
  chi += chi2_d0_mu_toy_test(data[2], "d0_mu_tag_mu", it, name, param[pos]);
  */
  //if(hists[pos] != nullptr) std::cout << hists[pos]->GetTitle() << std::endl;
  float chi = chi2_d0_mu_toy_test(data[1], "d0_mu_tag_mu", it, name, param[pos], iteration);
...
*/
float min = chiTest->GetFunction("pol3")->GetMinimumX(0.6,1.976);
...
float err = chiTest->GetFunction("pol3")->GetX(chimin+1,0.6,1.976);
...
shiftData->GetXaxis()->SetTitle("(D^{0} #it{p}_{T} + #mu #it{p}_{T}) / #Sigma_{ch} #it{p}_{T}");
delete mc2;
...
shiftData->Draw();
tdr(shiftData, epoch);
//c1->SaveAs(TString::Format("www/meson/morph/ptfrac/ptfrac_signal_toy_BCDEFGH_d0mu%d.pdf",iteration));
//c1->SaveAs(TString::Format("www/meson/morph/ptfrac/ptfrac_signal_toy_BCDEFGH_d0mu%d.png",iteration));
}
}
