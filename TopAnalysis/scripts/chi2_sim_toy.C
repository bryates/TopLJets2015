...
TString report("");
TString json("\"sim\" :      [");
float chi2_sim_toy_test(TH1F *&data, TH1F *& shiftData, TString sample="sim", TString tune="", TString name="", float num=0.855, int iteration=0);
//float chi2_sim_toy_test(TH1F *&data, TH1F *&mc, TString sample="sim", TString tune="", TString name="", float num=0.855);
void run_chi2_sim_toy(TString, int iteration);
//void run_chi2_sim_toy(std::vector<TH1F*>&, std::vector<TH1F*>&, TString);
RooRealVar ptfrac;
...
//fname = TString::Format("/afs/cern.ch/user/b/byates/TopAnalysis/LJets2015/2016/mtop/sPlot/sPlot/morph/TopMass_172v5%s_sPlot_sim.root",tune.Data());
else
...
/*
  run_chi2_sim_toy("isr-down" ndata, nmc);
  run_chi2_sim_toy("isr-up" ndata, nmc);
  run_chi2_sim_toy("fsr-down" ndata, nmc);
  run_chi2_sim_toy("fsr-up" ndata, nmc);
  run_chi2_sim_toy("uedown" ndata, nmc);
  run_chi2_sim_toy("ueup" ndata, nmc);
  //run_chi2_sim_toy("erdON" ndata, nmc);
  run_chi2_sim_toy("GluonMove_erdON" ndata, nmc);
  //run_chi2_sim_toy("QCD_erdON" ndata, nmc);
  ...
  for(auto & it : syst) {
    run_chi2_sim_toy("down_"+it ndata, nmc);
    run_chi2_sim_toy("up_"+it ndata, nmc);
  }
  run_chi2_sim_toy("hdampdown" ndata, nmc);
  run_chi2_sim_toy("hdampup" ndata, nmc);
  run_chi2_sim_toy("tpt" ndata, nmc);
  run_chi2_sim_toy("bkg" ndata, nmc);
  run_chi2_sim_toy("as117" ndata, nmc);
  run_chi2_sim_toy("as119" ndata, nmc);
  run_chi2_sim_toy("m166v5" ndata, nmc);
  run_chi2_sim_toy("m169v5" ndata, nmc);
  run_chi2_sim_toy("m171v5" ndata, nmc);
  run_chi2_sim_toy("m173v5" ndata, nmc);
  run_chi2_sim_toy("m175v5" ndata, nmc);
  run_chi2_sim_toy("m178v5" ndata, nmc);
*/
...
void run_chi2_sim_toy(TString name="", int iteration=0) {
//void run_chi2_sim_toy(std::vector<int> &nentries, std::vector<int> &nmc, TString name="") {
gROOT->Reset();
low=999.;
high=0;
//name=lname;
...
for(auto & it : tune) {
  ...
  /*
  float chi = chi2_sim_toy_test(data[0], "jpsi", it, name, param[pos]);
  chi += chi2_sim_toy_test(data[1], "sim", it, name, param[pos]);
  chi += chi2_sim_toy_test(data[2], "sim_mu_tag_mu", it, name, param[pos]);
  */
  //if(hists[pos] != nullptr) std::cout << hists[pos]->GetTitle() << std::endl;
  float chi = chi2_sim_toy_test(data[0], shiftData[0], "jpsi", it, name, param[pos], iteration);
  chi += chi2_sim_toy_test(data[1], shiftData[1], "d0", it, name, param[pos], iteration);
  chi += chi2_sim_toy_test(data[2], shiftData[2], "d0_mu_tag_mu", it, name, param[pos], iteration);
  //float chi = chi2_sim_toy_test(data[1], "d0_mu_tag_mu", it, name, param[pos], iteration);
...
if(tune == "_sdown") { //only compute modified hist once
  ...
  shift = sample.Contains("jpsi") ? 0 :
          sample.Contains("mu_tag") ? 2 : 1;
...
shiftData->SetLineColor(kBlack);
shiftData->SetLineWidth(2);
c1->SaveAs("ptfrac_signal_Data_"+name+"sim.pdf");
c1->SaveAs("ptfrac_signal_Data_"+name+"sim.png");
...
shiftData->GetXaxis()->SetRangeUser(0.125,0.975);
