...
TString report("");
float chi2_d0_mu_tag_test(TString mcTune="", TString tune="");
std::vector<RooChi2Var> chi;
RooRealVar ptfrac;

void calibration_d0_mu_tag() {
...
for(auto & mc : tune) {
  ...
  for(auto & it : tune) {
    ...
    //if(it == "_sdown" || it == "_uup" || it == "_up") continue;
    */
    std::cout << "Running on tune: " << it << std::endl;
    //if(abs(param[&it - &tune[0]] - param[&mc - &tune[0]]) > 0.1) continue;
    float chi = chi2_d0_mu_tag_test(mc, it);
    int pos = &it - &tune[0];
...
TH1F *curve = new TH1F("curve","#chi^{2} curve", 100, -1, 1);
TH1F *curveg = new TH1F("curveg","#chi^{2} curve", 40, 0.6, 1.1);
for(size_t i = 0; i < tune.size(); i++) {
  std::cout << "Closure test with " << tune[i] << std::endl;
  ...
  pt->Draw();
  gStyle->SetOptStat(0);
  c1->SaveAs(TString::Format("chi2_d0_mu_tag-calibration%s.png",tune[i].Data()));
  c1->SaveAs(TString::Format("chi2_d0_mu_tag-calibration%s.pdf",tune[i].Data()));
...
curve->GetXaxis()->SetRangeUser(-0.2,0.2);
curve->GetYaxis()->SetRangeUser(0.55,1.2);
curve->Draw();
TFitResultPtr f = curve->Fit("pol1","FS");
float avg_deltag(0.);
for(auto & it : param) {
  float itg(it-0.855);
  float calibm = f->Parameter(0) + f->Parameter(1)*itg;// + f->Parameter(2)*itg*itg;
...
for(auto & it : param) {
  float itg(it-0.855);
  float calibm = f->Parameter(0) + f->Parameter(1)*itg;// + f->Parameter(2)*itg*itg;
  std::cout << "r_B: " << it << " = " << calibm << " -> " << calibm - avg_deltag << " +/- " << sqrt(pow(f->ParError(0),2)+pow(f->ParError(1),2)) << std::endl;
  //std::cout << "r_B: " << it << " = " << calibm << " -> " << calibm - avg_deltag << " +/- " << sqrt(pow(f->ParError(0),2)+pow(f->ParError(1),2)+pow(f->ParError(2),2)) << std::endl;
}
curveg->Draw();
...
//TString text = TString::Format("Calibration curve : %0.2f (#pm%0.2f) %c (r_{B} - 0.855) %0.2f (#pm%0.2f)",f->Parameter(0),abs(f->ParError(0)),sign,f->Parameter(1),abs(f->ParError(1)));
...
pt->AddText(text);
pt->Draw();
c1->SaveAs("chi2_d0_mu_tag-calibration_curve.png");
c1->SaveAs("chi2_d0_mu_tag-calibration_curve.pdf");
}

float chi2_d0_mu_tag_test(TString mcTune="", TString tune="") {
TFile *fdata = TFile::Open(TString::Format("4dau/TopMass_172v5%s_sPlot_d0_mu_tag_mu.root",mcTune.Data()));
...
sigData = (RooDataSet*)wmc->data("sigData")->reduce("j_pt_ch<75");
