...
void plotJPsi_hdamp() {
/*
std::vector<TString> tune = {"", "up_PI", "_fit"};
...
std::vector<TString> tune = {"_sdown", "_700", "_down", "_central"};
std::vector<float> param = {0.655, 0.700, 0.755, 0.855};
*/
std::vector<TString> tune = {"_sdown", "_down", "_central"};
std::vector<float> param = {0.655, 0.755, 0.855};
...
TString name(TString::Format("LJets2015/2016/mtop/TopMass_hdampup%s_sPlot_jpsi1.root",tune[i].Data()));
if(tune[i].Contains("fsr"))
  name=TString::Format("LJets2015/2016/mtop/TopMass_%s_sPlot_jpsi1.root",tune[i].Data());
...
TFile *fin = TFile::Open("LJets2015/2016/mtop/TopMass_Data_sPlot_jpsi1.root");
tmp.push_back(convert((RooPlot*)fin->Get("ptfracJ_signal"),false,0,1.1));
//tmp.push_back(convert((RooPlot*)fin->Get("ptfrac_signal"),false,0,1.1));
tmp.back()->Scale(1./tmp.back()->Integral());
...
tmp.front()->SetTitle("J/#Psi p_{T} / jet p_{T}^{ch}");
//tmp.front()->SetTitle("J/#Psi p_{T} / #Sigma p_{T}^{ch}");
tmp.front()->GetXaxis()->SetTitle("J/#Psi p_{T} / jet p_{T}^{ch}");
